...
- M3.2 单 clip 数值门禁使用原 A800 FP32 CPUAdam profile，但关闭 checkpoint，避免 120 GiB 主机在保存时复制 FP32 state。该诊断把 `clean_action_ratio` 临时设为 0，使 batch size 1 的每个 step 都监督 action/proprio，并显式记录监督比例；M3.3 和正式训练恢复模型层默认值 0.5。
