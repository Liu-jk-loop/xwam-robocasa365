...
from data.robocasa365_contract import EXPECTED_ACTION_DIM, EXPECTED_STATE_DIM, inspect_dataset, load_task_manifest
...
class RoboCasa365Dataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        if invalid_types:
            raise ValueError(f"camera_types 只允许 static/dynamic，实际包含 {invalid_types}")
        ...
        self._parquet_cache: OrderedDict[int, tuple[np.ndarray, np.ndarray]] = OrderedDict()
        ...
        if not summary.ok:
            raise ValueError("RoboCasa365 数据契约失败：\n- " + "\n- ".join(summary.errors))
    ...
    def sample_paths(self, index: int) -> dict[str, Any]:
        clip = self.clip_spec(index)
        return {
            "parquet": str(episode_data_path(self.dataset_root, self.info, clip.episode_index)),
            "videos": {
                camera_key: str(episode_video_path(self.dataset_root, self.info, clip.episode_index, camera_key))
                for camera_key in self.camera_keys
            },
        }

    @staticmethod
    def _validate_video_size(video_size: list[int] | tuple[int, int]) -> tuple[int, int]:
        if len(video_size) != 2:
    ...
    def _validate_episode_files(self) -> None:
        ...
        for episode in self.episodes:
            data_path = episode_data_path(self.dataset_root, self.info, episode.episode_index)
            if not data_path.is_file():
                missing.append(str(data_path))
            for camera_key in self.camera_keys:
                video_path = episode_video_path(self.dataset_root, self.info, episode.episode_index, camera_key)
                if not video_path.is_file():
                    missing.append(str(video_path))
            if len(missing) >= 8:
                break
        if missing:
            raise FileNotFoundError("RoboCasa365 episode 文件缺失（最多展示 8 个）：\n- " + "\n- ".join(missing[:8]))

    @staticmethod
    def _column_to_matrix(table: Any, column_name: str, expected_dim: int, path: Path) -> np.ndarray:
        if column_name not in table.column_names:
            raise ValueError(f"Parquet 缺少列 {column_name!r}：{path}；实际列={table.column_names}")
        try:
            matrix = np.asarray(table[column_name].combine_chunks().to_pylist(), dtype=np.float32)
        except (TypeError, ValueError) as exc:
            raise ValueError(f"Parquet 列 {column_name!r} 无法转换为 float32 矩阵：{path}: {exc}") from exc
    ...
    def _load_episode_arrays(self, episode_index: int, expected_length: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        states = self._column_to_matrix(table, "observation.state", self.proprio_dim, path)
        actions = self._column_to_matrix(table, "action", self.action_dim, path)
        frame_index = np.asarray(table["frame_index"].combine_chunks().to_pylist(), dtype=np.int64)
        ...
        if not np.all(episode_indices == episode_index):
            raise ValueError(f"Parquet episode_index 与文件 episode {episode_index} 不一致：{path}")
    ...
    def _video_reader(self, episode_index: int, camera_key: str) -> VideoReader:
        ...
        path = episode_video_path(self.dataset_root, self.info, episode_index, camera_key)
        try:
    ...
    def _read_video_frames(self, episode_index: int, camera_key: str, frame_ids: np.ndarray) -> torch.Tensor:
