...
def validate_m6_formal_training_contract(
    ...
    checks = {
        ...
        == expected_num_nodes * expected_devices_per_node,
    ...
    return {
        ...
        "devices_per_node": expected_devices_per_node,
...
def _checkpoint_layout(
    path: str | Path, *, expected_world_size: int = 4
) -> dict[str, Any]:
    expected_world = int(expected_world_size)
    if expected_world <= 0:
        raise ValueError("expected_world_size 必须为正整数")
    ...
    checks = {
        ...
        "optimizer_shards": optimizer_ranks == list(range(expected_world))
...
def resolve_m6_formal_chunk(
    ...
    *,
    ...
    expected_world = int(expected_world_size)
    if total <= 0 or chunk <= 0 or expected_world <= 0:
        raise ValueError("total_steps/chunk_steps/expected_world_size 必须为正整数")
    ...
    for root_index, root in enumerate(roots):
        ...
        for candidate in sorted(root.iterdir()):
            ...
            layout = _checkpoint_layout(candidate, expected_world_size=expected_world)
            if all(layout["checks"].values()):
                final_priority = 1 if candidate.name.startswith("final-step=") else 0
                complete.append((step, final_priority, root_index, candidate.resolve()))
            else:
                incomplete.append(
                    {
                        "step": step,
...
def _optimizer_audit(
    directory: str | Path, *, expected_world_size: int = 4
) -> dict[str, Any]:
    expected_world = int(expected_world_size)
    if expected_world <= 0:
        raise ValueError("expected_world_size 必须为正整数")
    ...
    checks = {
        "rank_reports": len(reports) == expected_world
        and ranks == list(range(expected_world)),
        "reported_world_size": bool(reports)
        and all(
            int(report.get("world_size", expected_world)) == expected_world
...
def _run_contract(
    *,
    ...
    checkpoint = (
        _checkpoint_layout(
            completed_checkpoint, expected_world_size=expected_world_size
        )
    ...
    checks = {
        ...
        "checkpoint_layout": bool(checkpoint) and all(checkpoint["checks"].values()),
    ...
    *, initial: dict[str, str], resumed: dict[str, str]
    ...
    try:
        ...
        cross_checks = {
            ...
            "resume_uses_initial_checkpoint": bool(resumed_report["resume_checkpoint"])
            and str(Path(resumed_report["resume_checkpoint"]).expanduser().resolve())
